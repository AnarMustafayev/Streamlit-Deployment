...
icon = Image.open("icon.jpg") #saving pictures as variable
...
**Potability** - Target variable; indicates water potability with values 1 (potable) and 0 (not potable).""")
    ...
    def describe_data(df):
        ...
        if p2.button("Data preprocessing"):
            cat_cols = df.iloc[:,:-1].select_dtypes(include="object").columns  #categotical columns
            ...
            if cat_cols.size > 0:
                if cat_m == "Mode":
                    ...
                    df[cat_cols].fillna(method = "ffil",inplace=True) #fill with previous value in the column
